...
class UNext(nn.Module):
    ...
    def __init__(self,  num_classes, input_channels=3, deep_supervision=False,img_size=224, patch_size=16, in_chans=3,  embed_dims=[128, 160, 256],
        ...
        self.encoder1 = nn.Conv2d(16, 16, 3, stride=1, padding=1)  
...
#### new ###########
        ...
        self.final_bn = nn.ModuleList([nn.BatchNorm2d(16),
                                       nn.BatchNorm2d(16),
                                       nn.BatchNorm2d(3),
                                       nn.BatchNorm2d(3),
                                       nn.BatchNorm2d(3),
...
#### new ###########
    ...
    def forward(self, x):
        ...
        # out = F.interpolate(out, size=(248,436), mode='bilinear')
        # out = self.final(out)

        out = F.interpolate(out, size=(244,432), mode='bilinear')


        # r = self.final[0](out)
        # out = out + r
        # out = self.final_bn[0](out)

        # r = self.final[1](out)
        # out = out + r
        # out = self.final_bn[1](out)
        ...
        # r = self.final[-3](out)
        # out = out + r
        # out = self.final_bn[3](out)

        # r = self.final[-2](out)
        # out = out + r
        # out = self.final_bn[4](out)


        # r = self.final[-1](out)
        # out = out + r
        # out = self.final_bn[5](out)
